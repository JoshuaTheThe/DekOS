...
{
        k_print("Checking for CD-ROM at port %d, slave: %d, ", port, slave);

        if (!cdrom_detect(port, slave))
        {
                k_print("Could not find\n");
                return 0;
        }

        if (cdrom_media_present(port, slave))
        {
                k_print("CD media is present\n");
                return 2;
        }
        else
        {
                k_print("CD media not present\n");
                return 1;
        }
}
...
void cd_drive_init(int *port, bool *slave)
{
        ...
        if (*port == 0 && *slave == 0)
        {
                k_print("Failed to find CD-ROM\n");
                return;
        }

        //k_print("Primary channel status: %d; ", primary_status);
        //k_print("Secondary channel status: %d\n", secondary_status);
        k_print("Port: %d; IsSlave: %d\n", *port, *slave);
}
