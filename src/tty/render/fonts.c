...
font_t *FontLoad(const char *path)
{
        ...
        // Open the font file
        file = FOpen(path, strnlen(path, MAX_PATH), FILE_PRESENT | FILE_READABLE);
        if (!file)
        {
        ...
        if (!buffer)
        {
                printf(" [ERROR] Could not allocate buffer for font\n");
                goto cleanup;
        }

        // Read file data
        size_t read = FRead((char*)buffer, 1, file->size, file);
        if (read != file->size)
        {
        ...
        {
                printf(" [ERROR] Invalid font magic\n");
                goto cleanup;
        }
        ptr += 4;
        ...
        if (version != 1)
        {
                printf(" [ERROR] Unsupported font version: %d\n", version);
                goto cleanup;
        }

        // Read font properties
        uint32_t char_width = *(uint32_t *)ptr;
        ptr += 4;
        uint32_t char_height = *(uint32_t *)ptr;
        ptr += 4;
        uint32_t first_char = *(uint32_t *)ptr;
        ptr += 4;
        uint32_t last_char = *(uint32_t *)ptr;
        ptr += 4;
        uint32_t bpp = *(uint32_t *)ptr;
        ptr += 4;
        uint32_t data_size = *(uint32_t *)ptr;
        ptr += 4;

        // Read font name (null-terminated)
        const char *name = (const char *)ptr;
        size_t name_len = strnlen(name, 256);
        ptr += name_len + 1;

        if (data_size != file->size - (ptr - buffer))
        {
                printf(" [ERROR] Font data size mismatch\n");
                goto cleanup;
        }
        ...
        if (!font)
        {
                printf(" [ERROR] Could not allocate font structure\n");
                goto cleanup;
        }

        uint8_t *bitmap = malloc(data_size);
        if (!bitmap)
        {
                printf(" [ERROR] Could not allocate font bitmap\n");
                free(font);
                font = NULL;
                goto cleanup;
        }
        ...
        unsigned char *font_name = malloc(name_len + 1);
        if (font_name)
        {
        ...
        printf(" [INFO] Loaded font: %s (%dx%d, %d chars, %d bpp)\n",
               font_name, char_width, char_height,
               last_char - first_char + 1, bpp);
