...
static gdtTssEntry_t tss[MAX_PROCS];
...
void gdtTssInit(void)
{
        ...
        tss[0].esp0 = 0x10000;
        tss[0].ss0 = 0x10;
        tss[0].iomap_base = 0xFFFF;  // Allow all I/O ports
}
...
void gdtInit(void)
{
        ...
        {
                uint32_t tss_base = (uint32_t)&tss[i];
                gdtSetTssEntry(3 + i, tss_base, sizeof(tss[0]) - 1, 0x89, 0x40);
        }
